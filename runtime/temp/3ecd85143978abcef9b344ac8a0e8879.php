<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:75:"/www/wwwroot/yun.chinaeaglepower.cn/application/daili/view/siyou/index.html";i:1558461582;s:77:"/www/wwwroot/yun.chinaeaglepower.cn/application/daili/view/public/header.html";i:1551521020;s:77:"/www/wwwroot/yun.chinaeaglepower.cn/application/daili/view/public/footer.html";i:1536907466;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="<?php if($ismobile==0): ?>wrapper wrapper-content<?php endif; ?> animated fadeInRight">
    ...
    <div class="ibox float-e-margins">
        ...
        <div class="ibox-content">
          ...
          <a href="<?php echo url('',['type'=>0]); ?>"><button class="btn btn-outline btn-primary" type="button" style="<?php echo $typess?'':'background-color: #1ab394;
    border-color: #1ab394;
    color: #FFF;'; ?>">全部</button></a>
                                <?php foreach($type as $key=>$vo): ?>
							<a href="<?php echo url('',['type'=>$vo['id']]); ?>"><button class="btn btn-outline btn-primary" type="button"  style="<?php echo $typess==$vo['id']?'background-color: #1ab394;
    border-color: #1ab394;
    color: #FFF;':''; ?>"><?php echo $vo['name']; ?></button></a> 
                                <?php endforeach; ?>
						</div>
...
<?php if(\think\Config::get('dlyulan')==1): ?><a class="btn2"  onClick="play('{{d[i].url}}')" style="width:5%;"><i style="font-size:25px" class="fa fa-play-circle-o"></i></a><?php endif; else: ?>
									{{d[i].name}}	
									<?php endif; ?>
</td>  
									<?php if($ismobile==0): ?>
                                   ...
                                   <td>{{d[i].shijian}}</td>      
                                    <td>{{d[i].cs}}</td>
                                   <td><a href='{{d[i].dwz}}'  target='_blank'>{{d[i].dwz}}</a>   <span onClick="ewm('{{d[i].dwz}}')" title='点击弹出二维码'>点击弹出二维码</span></td>
...
function ewm(url) {
layer.alert('<div class="text-center pd20">\<p><img src="http://api.k780.com:88/?app=qr.get&data='+url+'" class="img-responsive wp shoukuan" style="width:100%;"></p>\</div>\
', {
...
<script type="text/javascript">
   ...
   $(document).on('change', '.price', function(event) {
    ...
    function Ajaxpage(curr){
        var key=$('#key').val();
        $.getJSON('<?php echo url("siyou/index",['type'=>$typess]); ?>', {
            page: curr || 1,key:key
