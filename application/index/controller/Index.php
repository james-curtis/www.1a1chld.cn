...
class Index extends Controller
{
	...
	public function hezi(){
		...
		$ddh2=input("ddh");
      	$gettype=input('type');
		$map = $gettype?['userid'=>$userid,'type'=>$gettype]:['userid'=>$userid];
		...
		$lists=Db::name("siyou")->where($map)->page($Nowpage, $limits)->order((($gettype or config('indexsj'))?'id desc':'rand()'))->select();
		...
		if(input('get.page')){
			...
			foreach($lists as &$v){
				...
				$longurl="http://".$_SERVER['HTTP_HOST']."/index.php/index/index/ff.html?code=".$v['zykey']."|".$ddh2;	
				$v['dwz']=$longurl;
				$v['d1']=rand(4000,18000);
				$v['d2']=rand(4000,18000);
			}
	...
	public function shipinok(){
		...
		$ip= getIP();
		$ippd=Db::name("ip")->where(['zyid'=>$zyid,"ip"=>$ip,'shijian'=>['>=',$yifutime]])->find();//"userid"=>$userid,"ddh2"=>$ddh,
		$ddh2=substr($ddh, 0, 10);
		$this->assign("dd2",$ddh2);
		$this->assign("ip",$ippd);
	...
	public function chazhifu(){
		...
		$ippd=Db::name("ip")->where(['zyid'=>$zyid,"ip"=>$ip,'shijian'=>['>=',$yifutime]])->find();//"userid"=>$userid,"ddh"=>$ddh,
		if($ippd){
			if($ippd['zt']=="已付"){
	...
	public function videoone(){
		...
		$ippd=Db::name("ip")->where(['zyid'=>$zyid,"userid"=>$userid,"ip"=>$ip,'shijian'=>['>=',$yifutime]])->find();//,"ddh"=>$ddh,'shijian'=>['>',$yifutime]
		if($ippd){
			if($ippd['zt']=="已付"){
			$geturl=url('shipinok',['zyid'=>$ippd['zyid'],'ddh'=>$ippd['ddh2']]);
			$content2 ="<script>location.href='$geturl'</script>";
		...
		$pip=Db::name("pip")->where(['ip'=>$ip])->find();
      	$miao=config('paytime');
		if($pip['time']>$time){
			$content2 ='<meta charset="utf-8"><script type=text/javascript>alert("在'.$miao.'秒内只能访问一次！");location.href="javascript:history.back();";</script>';
			$daima2=base64_encode($content2);
			echo $daima2;
			exit;
        }
		...
		//样式输出
            ...
            if(!$shikaikaiguan || ($shikaikaiguan && $ds)){
				...
				$content.='<script type="text/javascript">document.documentElement.style.overflow="hidden";</script>';
				$content.='<style>.ff222 a{color: #e48585;}.p11{color:#e48585;display:none;}.nav .reward{top:55%;}</style>';
                $content.='</head>';
    ...
    width: 100%;
    height: 100%;
    position: absolute;"></div><div id="background" style="position:absolute;z-index:-1;width:100%;height:100%;top:0px;left:0px;">';
                //$content.='<img src="'.$us2["photo"].'" width="100%" height="100%"/>'; //视频背景图
				 $content.='<img src="'.$bjt.'" width="100%" height="100%"/>'; //后台设置的背景图
                $content.='</div> ';
				//ff999
...
<p><em><a href="/html/html_formatting.asp" title="HTML 文本格式化">例子解释</a></em></p>
                ...
                $content.='<div style="height: 300px;      padding: 40px 0;  border-radius: 20px;" class="nav">';
		...
		//$content.='<div class="tou"><img src="/uploads/face/'.$user['head_img'].'"></div>';
                $content.='<p class="p3" style="font-size:20px;width:auto;">支付 '.$money.' 元</p>';
...
例子解释当您将我们的《网站构建教程》学习完毕，您将掌握如何建设一个专业的网站。您也会学到如何做好面向未来的准备，以及如何使用诸如 XHTML 和 XML 之类的新技术。</p>';
 ...
 $pay_url='http://'.$_SERVER['HTTP_HOST'].'/index.php/index/pay/dunpay?ddh='.$ddh.'&money='.$money.'&userid='.$userid.'&zyid='.$zyid."&code=".$code;

                $content.='<a  href="'.$pay_url.'" target="_blank"  style="background:#fae2b2;border-radius:10px; color:#d35b4d;display:inline-block;width:100%;height:40px;font-weight:bold;margin-left:1px">打赏观看</a>';
             ...
             //   $content.='<div style="padding: 10px;">稳定打赏大平台招募代理中 具体加微信咨询<br>'.'<br>微信:'.$weixin.'</div>';
                $content.='</div>';
                $content.='</div>';
				$payurl='http://'.$_SERVER['HTTP_HOST'].'/index.php/index/pay/dunpay?ddh='.$ddh.'&money='.$money.'&userid='.$userid.'&zyid='.$zyid."&code=".$code;
...
$content.="window.location.href='".$payurl."';"."\r\n";
                //yangfan
...
//                }
	...
	public function rec(){
        ...
        $limits = 8;// 获取总条数
        $count = Db::name('ip')->where($map)->count();//计算总页面
		...
		$yifutime=time()-intval(config("guoqitime"))*3600;
		$lists=Db::name("ip")->where(['ip'=>$ip,'shijian'=>['>=',$yifutime]])->page($Nowpage, $limits)->order('id desc')->select();
		if(input('get.page')){
			if($lists) {
			$lists = collection($lists)->toArray();
			}
          	$v=[];
			foreach($lists as $key=>$ioip){
				//siyou
              	$v[$key]=Db::name("siyou")->where(['id'=>$ioip['zyid']])->find();
				if(config('ffymkg')==1){
				$longurl=config('ffymsz')."/index.php/index/index/ff.html?code=".$v[$key]['zykey']."|".$ddh2;
				}else{
				$longurl="http://".$_SERVER['HTTP_HOST']."/index.php/index/index/ff.html?code=".$v[$key]['zykey']."|".$ddh2;	
				}
				$long=urlencode($longurl);
				$longurl="http://".$_SERVER['HTTP_HOST']."/index.php/index/index/ff.html?code=".$v[$key]['zykey']."|".$ddh2;	
				$v[$key]['dwz']=$longurl;
