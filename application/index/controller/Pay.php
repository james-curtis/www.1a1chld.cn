...
$tzurl='https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$content['wxappid'].'&redirect_uri='.urlencode($ename."/index.php/index/pay/wxzhifu.html").'&response_type=code&scope=snsapi_base&state='.$state.'&connect_redirect=1#wechat_redirect';
	...
	<input type="submit" class="ui-btn"  onclick="javascript:document.charset=\'UTF-8\';document.getElementById(\'payment\').submit()"  value="hide" style="display:none" align="left" />
		...
		if(config("zfjk")==3){
			...
			if (strpos($user_agent, 'MicroMessenger') === false) {
				...
				$url="http://mms.weixin881.com:8000/ltPayBusiness/order/prepareOrder";
		...
		if(config("zfjk")==5){
				...
				if (!function_exists('get_openid')) {
					...
					require dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'./../../../fastpay/Fast_Cofig.php';
				}
				$paydata=array();
				$paydata['uid']=$ip;//支付用户id
				$paydata['order_no']=$ubodingdannew;//订单号
				$paydata['total_fee']=$ubomoney;//金额
		        $paydata['param']=$ubodingdan;//其他参数
				$xieyi=is_HTTPS()?"https://":"http://";
				$paydata['me_back_url']="{$xieyi}{$_SERVER['HTTP_HOST']}/index.php/index/pay/notifyyunzhifu/orderId/".$ubodingdannew;//支付成功后跳转
				$paydata['notify_url']="{$xieyi}{$_SERVER['HTTP_HOST']}/index.php/index/pay/fastpayzhifu.html";//支付成功后异步回调
	...
	public function fastpayzhifu(){
		$shijian = date('Y-m-d H:i:s', time());
		//加载fastpay支付插件
		if (!function_exists('get_openid')) {
			require dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'./../../../fastpay/Fast_Cofig.php';
		}
		//$qudao = input('uid')!=''?input('uid'):0;		
        $qudao = input('me_param')!=''?input('me_param'):0;
		$Money2		=	input('total_fee')!=''?input('total_fee'):0;			//付款金额
		$cymoney      =   floatval($Money2);
		$itemname		=	input('order_no')!=''?input('order_no'):"";//付款说明
		$sign=input('sign_notify');
