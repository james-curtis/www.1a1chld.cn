...
$dbconfig=require dirname ( __FILE__ ).DIRECTORY_SEPARATOR.'./../application/database.php';
try {
...
$rows=$DB->query("SELECT * FROM {$dbconfig['prefix']}zhifu WHERE id='1' limit 1")->fetch();
$row=unserialize($rows['content']);
define("PAY_VERSION", '3');//版本
define("PAY_VERSION_P", '2.10');//版本
//FAST客户支付配置
define("FAST_APPKEY", $row['fAppkey']);//你的appkey
define("SECRET_KEY", $row['fSECRET_KEY']);//你的秘钥
define('PAY_DESC', "xx平台给你汇款");//汇款备注
define('PAY_RETURN_URL', "http://".$_SERVER['HTTP_HOST']);//支付后成功后返回的页面,
define('PAY_NOTIFY_URL', "http://{$_SERVER['HTTP_HOST']}/index.php/index/pay/fzhifu.html");//异步回调页面,不填写默认就是会员中心的---优先顺序-下单时候传入优先--->这里的回调----->会员中心的
