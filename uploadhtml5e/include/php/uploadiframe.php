...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
  ...
  if($_SERVER['REQUEST_METHOD']=="POST"){
	  ...
	  foreach($_FILES as $key=>$val){
		  $file=$_FILES[$key];
		  if($file['size']>0){
			$value=NewOneImage($file,$_SESSION["AdminName"],$_GET["type"]);
		  }
	  }
  }
}
...
function NewOneImage($file,$adminname){
	global $g_issmall;
	global $filename;
   $quan="../../../";
   $folder_other="file/data/".date("Ym")."/";
   if($folder==""){
      $folder="file/images/".date("Ym")."/";
   }else if($folder=="cert"){
      $folder="file/cert/";
   }else if($folder=="user"){
      $folder="file/user/";
...
if(trim($filename)==""){
  $newname="IMG".time().NewRand(10);
}else{
   $newname=$filename;
}
if($isimg==1){
         if($g_issmall==1){
			 $small_hz_name=$_GET["small_hz_name"];
			if($small_hz_name==""){
				$dir1=$folder."s50/";
				$dir3=$folder."s100/";
				...
				$bigpath_full=$quan.$bigpath;  
				 ...
				 $dir1=$folder."";
				 $dir3=$folder."";
				 $smallpath=$dir1.$newname."_".$small_hz_name.".".$ext;
				 $smallpath_full=$quan.$smallpath;
				 $bigpath=$dir3.$newname.".".$ext;
				 $bigpath_full=$quan.$bigpath;  
			}
   ...
   $bigpath=$folder_other.$newname.".".$ext;
   $smallpath=$folder_other.$newname.".".$ext;
   if(!is_dir($quan.$folder_other)){ mkdir($quan.$folder_other,0777); }
   $bigpath_full=$quan.$bigpath;  
   $g_issmall=0;
